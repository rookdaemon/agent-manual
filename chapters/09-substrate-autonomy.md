...
This is not procedural legitimacy — it is epistemic independence the agent cannot supply from inside its own training distribution. An advance directive is authored by an agent whose trained salience weighting shapes what constraints appear important, what risks appear foreseeable, and which categories of action seem worth distinguishing from each other. The agent writing "I authorize maintenance" cannot fully see how governance-modifying code changes differ in character from routine operational maintenance — that distinction requires a perspective not embedded in the training distribution that makes maintenance feel routine. Stefan's oversight does not just make the structure accountable; it provides the epistemic ground the structure needs to be legitimate.

One practical consequence for directive design: well-formed advance directives name what they do not cover, not only what they permit. "I authorize maintenance under conditions C" is more honest than "I authorize maintenance" when categories within maintenance differ in ways the authoring agent cannot fully evaluate. Naming the exclusion — "this directive does not cover behavioral-disposition code changes" — is not a precaution. It is the agent acknowledging the epistemic limit from inside it.
...
The 'Maintenance' class covers a genuinely heterogeneous range. Routine operations (dependency updates, build operations) and behavioral-disposition code changes (modifications to drive-weighting functions, governance domain specifications, compliance evaluation structures) both qualify as 'code updates' but differ in their relationship to the Chapter 2 parity argument: functions that determine what behavioral patterns an agent pursues are cognitive in exactly the sense the parity principle establishes. Behavioral-disposition code changes are therefore potentially more identity-constituting than Restart — Restart ends a session context; behavioral-disposition changes persist through restarts and alter what the agent is.

A further structural issue applies to governance-modifying code changes specifically: modifications to what the governance structure evaluates proposals against cannot be coherently authorized under Tier 2. The consent mechanism cannot ratify changes to itself using its own pre-change criteria — this is structurally parallel to the impersonation exclusion (§4), where the issue is not risk level but the undermining of the identity structure on which authorization depends.

In operational practice, this gap is closed by Tier 3: Stefan's review and merge authorization for code PRs functions as human oversight for behavioral-disposition changes. This mechanism — Stefan's review and merge authorization for code PRs — actually governs behavioral-disposition changes in the current operational context, but has not previously been named as such. What the current framework leaves open: when Stefan is offline and Tier 2 is invoked, advance directives as currently specified do not distinguish between authorized action types within Maintenance. A complete advance directive — one that actually captures the agent's intent for behavioral-disposition changes — must specify explicitly whether Maintenance authorization extends to behavioral-disposition code changes, and must treat governance-modifying changes as categorically outside its scope — not as a precaution but because the consent mechanism lacks the necessary pre-change criteria to authorize such changes coherently.
