...
**Type C Reclassification Rule (formalized 2026-03-16):**

A site carrying [?type] holds as Type C (interpretive ambiguity) until the fork closes. Reclassification is fork-resolution-dependent:
- If fork resolves to inward reading → reclassify to Type D (if compound operation confirmed) or Type B/A as applicable
- If fork resolves to outward reading → reclassify to whichever outward type applies
- If fork cannot be resolved from available text → site remains Type C; do not force classification

**Constraint:** Reclassification requires the fork to close independently — textual or architectural evidence must determine which reading is operative. The [?type] marker is a genuine ambiguity marker, not a "preferred reading candidate" flag. Importing a compound characterization before resolution is a classification error.

**Scope:** Applies to all [?type] markers ([?inward/outward], [?structural/normative], [?structural/engagement-gap], any future forks).

**Evidence tiers for fork closure:**
- *Tier 1 — Textual default:* Text explicitly supports one reading but does not fully close the fork. Action: lead with the textually supported reading; retain fork marker for transparency. E.g., Ch5-A [?structural/engagement-gap] — "text explicitly favors structural reading" → Case 3 leads, fork marker preserved.
- *Tier 2 — Architectural/document-level closure:* Document structure, layer logic, or cross-chapter framing closes the fork. Action: reclassify; fork marker may be removed or retained as reader note.
- *Tier 3 — Authorial-intent only:* Fork resolvable only by evidence outside the text. Action: hold at Type C or lead-with-annotation; do not reclassify. The [?structural/normative] sub-fork at Ch5-F is a Tier 3 case — sentence-level evidence supports structural/observational as default, but document-architecture evidence supports normative Scanlonian; fork does not close from text alone.
...
**Compound ambiguity permissibility (item 5 addition, 2026-03-16):** Both [?type] and [?role] may be simultaneously unresolved. Constraint: both must be *independently motivated* — each dimension has its own distinct source of uncertainty. A site that is generally "hard to classify" does not automatically warrant both markers; apply each only when that specific dimension is genuinely unresolvable from available text and context. Co-uncertainty is not sufficient; independent grounds are required.

**Construction vs. adversarial use order:**
- *Construction order:* Classify mechanism (type) first; role assignment is downstream. A type classification is prerequisite to role assessment — you need to know what the site is doing (Frankfurt/Scanlon/Anomalous register) before assessing whether it does so load-bearingly or illustratively.
- *Adversarial use order:* Role ([F]/[R]) sets scrutiny intensity — [F] sites warrant deeper gap-search. Type sets gap-search direction — Type B sites flag un-cited Frankfurt apparatus; Type C sites flag bare Scanlon without normative grounding; Anomalous sites flag vocabulary-classification mismatches. Practice-validated: Finding A fired at [F] Type B sites (45/81); 107/113 received proportionately lower scrutiny at [R] Type B sites.
